...
type PreParseValidator = func(string) error

type PostParseValidator[T any] = func(T, string) error
...
// be returned.
func OneOfPreParseArgumentValidator(values []string, message string) PreParseValidator {
...
// returned.
func OneOfPostParseArgumentValidator[T comparable](values []T, message string) PostParseValidator[T] {
...
func NoneOfPreParseArgumentValidator(values []string, message string) PreParseValidator {
	return func(s string) error {
		for _, value := range values {
			if s == value {
				return errors.New(message)
...
// If the parsed value does match one of the values in the given values slice,
// the given message will be used to build a new error which will be returned.
func NoneOfPostParseArgumentValidator[T comparable](values []T, message string) PostParseValidator[T] {
	return func(a T, s string) error {
		for i := range values {
			if a == values[i] {
				return errors.New(message)
...
func NumericRangePostParseArgumentValidator[T NumericValue](minimum, maximum T, message string) PostParseValidator[T] {
	return func(a T, s string) error {
