...
func MapEntry(parser *mapElementParser) (k string, v string, err error) {
	if parser.HasNext() {
		k = parser.Next()
	} else {
...
func newMapElementParser(props argo.UnmarshalMapProps, rawValue string) mapElementParser {
	return mapElementParser{properties: props, raw: rawValue}
}

type mapElementParser struct {
	properties     argo.UnmarshalMapProps
	raw            string
	position       int
	expectingValue bool
}
